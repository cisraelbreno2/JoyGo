...
static void move_mouse_analog(int x, int y) {
    ...
    if (deltaX == 0.0f && deltaY == 0.0f) {
        ...
        CGWarpMouseCursorPosition(CGPointMake(location.x + prevSpeedX, location.y + prevSpeedY));
        return;
    }
    ...
    CGWarpMouseCursorPosition(CGPointMake(location.x + newSpeedX, location.y + newSpeedY));
}
